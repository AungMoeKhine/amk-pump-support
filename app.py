...
st.markdown(f"""
    <style>
        ...
        /* Ensure invisible iframes don't block mobile taps */
        iframe {{
        ...
        [data-testid="stChatMessage"] {{
            ...
            /* Webkit prefix for iOS Safari and hardware acceleration */
            -webkit-backdrop-filter: blur(8px);
            backdrop-filter: blur(8px);
            transform: translateZ(0); 
            
            /* Bring chat messages to the front so they can be clicked */
            position: relative;
            z-index: 99;
...
# This component injects JavaScript into the parent window
components.html(
    """
    <script>
    ...
    // --- 2. Android WebView Link Fix ---
    // Change all links from _blank (new tab) to _self (same tab). 
    // This allows your new Android MainActivity Java code to catch the click!
    const fixLinks = () => {
    ...
    fixLinks(); // Run once for existing links

    // Use a MutationObserver to instantly fix new links generated by the AI as it streams text
    const observer = new MutationObserver(() => {
        fixLinks();
    });
    
    // Watch the Streamlit app body for any new chat messages appearing
