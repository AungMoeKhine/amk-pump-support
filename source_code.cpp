...
<script>
  ...
  window.onload = () => {
    fetch('/config').then(r=>r.json()).then(c=>{
       ...
       let dD=""; for(let i=30; i<=180; i+=5) dD+=`<option value="${i}" ${c.dD==i?"selected":""}>${i} Seconds</option>`; document.getElementById('dD_s').innerHTML = dD;
...
String processManualToggle() {
  ...
  if (xSemaphoreTakeRecursive(systemMutex, pdMS_TO_TICKS(500))) {
    ...
    if (dryRunConfig.error != 0) {
      ...
      // Force hardware pins to safe state immediately
      digitalWrite(BUZZER_PIN, LOW); 
      digitalWrite(MOTOR_PIN, LOW);
      ...
      Serial.println("[USER] Dry-Run Alarm Reset & Buzzer Cleared.");
      xSemaphoreGiveRecursive(systemMutex);
      pendingMqttPublish = true;
      
      // Small hardware settling delay
      delay(50); 
      return "Success";
    }
...
void handleBuzzerPatterns() {
  ...
  // High Priority: Dry-Run Alarm beeps
  if (currentState == PumpState::DRY_RUN_ALARM) {
    digitalWrite(BUZZER_PIN, (now / 500) % 2);  // Oscillating Beep
  } 
  // Medium Priority: Sensor Error beeps
  else if (currentState == PumpState::SENSOR_ERROR && !tankConfig.errorAck) {
    unsigned long cycle = now % 5000;
    if (cycle < 600) digitalWrite(BUZZER_PIN, (cycle / 100) % 2);
    else digitalWrite(BUZZER_PIN, LOW);
  } 
  // Low Priority: Voltage Error (Disabled buzzer)
  else if (currentState == PumpState::VOLTAGE_ERROR) {
    digitalWrite(BUZZER_PIN, LOW);
  }
  // SAFETY: If not in any Alarm state, FORCE the buzzer OFF immediately.
  // This prevents the sound from getting stuck when the motor starts.
  else {
    digitalWrite(BUZZER_PIN, LOW);
  }
}
...
void updatePumpLogic() {
  ...
  switch (currentState) {
    case PumpState::PUMPING:
      ...
      // Safety: Force buzzer off while pumping is active
      if (dryRunConfig.error == 0) digitalWrite(BUZZER_PIN, LOW); 
